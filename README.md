...
当前仓库展示的是可运行原型、系统设计与验证记录，不代表已经获得外部用户采用或产生业务结果。

## 核心能力

- 今天与 28 天全景展示已安排任务；待安排任务先可靠收下，灵感库保存想法与学习问题。
...
如需启用本地 AI 代理，将 `.env.example` 复制为 `.env`，仅在服务端填写 `DEEPSEEK_API_KEY`，在一个终端运行 `npm run ai:proxy`，另一个终端运行 `npm run dev:ai`，让开发界面显式启用本地整理代理。不要把 `.env` 提交到 Git。
