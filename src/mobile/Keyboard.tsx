...
export function useKeyboardInsets() {
  ...
  const { device } = useMobileDevice();
  const reservesBottomNavigation = device.reservesBottomNavigation && !keyboard.visible;

  return {
    keyboardHeight: keyboard.height,
    keyboardFullHeight: keyboard.fullHeight,
    keyboardDragging: keyboard.isDragging,
    bottomInset: reservesBottomNavigation
      ? 0
      : Math.max(device.geometry.safeArea.bottom, keyboard.height),
    availableHeight:
      device.geometry.screen.height -
      keyboard.height -
      (reservesBottomNavigation ? device.geometry.safeArea.bottom : 0),
...
function GenericKeyboard() {
  return (
    <div className="generic-keyboard" aria-hidden="true">
      <div className="generic-keyboard-suggestions">
        <span>Plan</span><span>Focus</span><span>Done</span>
      </div>
      {KEY_ROWS.map((row, rowIndex) => (
        <div className="generic-keyboard-row" data-row={rowIndex + 1} key={rowIndex}>
          {row.map((key) => (
            <span className="generic-key" data-key={key} key={key}>
              {key === "UP" ? "↑" : key === "BACK" ? "⌫" : key}
            </span>
          ))}
        </div>
      ))}
      <div className="generic-keyboard-row generic-keyboard-row--utility">
        <span className="generic-key generic-key--utility">123</span>
        <span className="generic-key generic-key--space">space</span>
        <span className="generic-key generic-key--utility">return</span>
      </div>
    </div>
  );
}
