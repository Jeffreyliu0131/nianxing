...
async function drag(page: Page, locator: Locator, deltaX: number, deltaY: number, steps = 8, start?: { x: number; y: number }) {
  const box = await locator.boundingBox();
  if (!box) throw new Error("Drag target has no bounding box");
  const startX = box.x + (start?.x ?? box.width / 2);
  const startY = box.y + (start?.y ?? box.height / 2);
...
test("keyboard and its attached footer dismiss on the same transition", async ({ page }) => {
  ...
  // The visible flag changes before the opening motion settles. Start the
  // gesture at the displayed footer, and use its padding rather than its input.
  await expect.poll(() => footer.evaluate((element) => {
    const dock = document.querySelector<HTMLElement>('[data-testid="keyboard-dock"]')!;
    return Math.abs(Number.parseFloat(getComputedStyle(element).bottom) - Number.parseFloat(dock.style.height));
  })).toBeLessThan(1);
  await drag(page, input, 0, 110, 8);
  await expect(keyboard).toHaveAttribute("data-visible", "true");
  await drag(page, footer, 0, 180, 8, { x: 8, y: 8 });
  ...
  const settledInsets = await page.evaluate(() => {
    const footerElement = document.querySelector<HTMLElement>('[data-testid="flow-fixed-footer"]')!;
    const screenElement = document.querySelector<HTMLElement>('[data-testid="device-screen"]')!;
    return {
      footerBottom: Number.parseFloat(getComputedStyle(footerElement).bottom),
      safeAreaBottom: Number.parseFloat(
        getComputedStyle(screenElement).getPropertyValue("--device-safe-area-bottom"),
      ),
    };
  });
  expect(settledInsets.footerBottom).toBe(settledInsets.safeAreaBottom);
});

test("switching to Tall keeps the composer above the reserved navigation rail", async ({ page }) => {
  await page.goto("/tests/runtime-fixture.html?fixture=keyboard");
  const input = page.getByLabel("Message");
  ...
  await page.getByTestId("device-option-tall").click();

  const frame = page.getByTestId("phone-frame");
  const screen = page.getByTestId("device-screen");
  const statusIndicators = page.getByTestId("status-indicators");
  const navigation = page.getByTestId("navigation-rail");
  const footer = page.getByTestId("flow-fixed-footer");

  await expect(frame).toHaveAttribute("data-device", "tall");
  await expect(screen).toHaveAttribute("data-device", "tall");
  await expect(page.locator(".phone-bezel")).toHaveCount(0);
  await expect(page.getByTestId("device-sensor")).toBeVisible();
  await expect(statusIndicators).toHaveAttribute("data-device", "tall");
...
test("FlowStack pushes and pops screens while dismissing the keyboard", async ({ page }) => {
  ...
  expect(safeHeaderPlacement).toBeGreaterThanOrEqual(50);
